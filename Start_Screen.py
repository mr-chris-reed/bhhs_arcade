...
class Start_Screen:
    ...
    def draw_text(self, text, font_name, color, size, x, y, counter, divisor): # if no flashing wanted, enter 1 for divisor
        if divisor == 1:
            font = pygame.font.Font(font_name if font_name else None, size)
            text_surface = font.render(text, True, color)
            text_rect = text_surface.get_rect(center=(x, y))
            self.surface.blit(text_surface, text_rect)
        elif counter % divisor > 1 and counter % divisor < 30: 
            font = pygame.font.Font(font_name if font_name else None, size)
            text_surface = font.render(text, True, color)
            text_rect = text_surface.get_rect(center=(x, y))
            self.surface.blit(text_surface, text_rect)
    ...
    def generate_return_surface(self, counter):
        ...
        self.draw_text("Press A to Start", "fonts/PirataOne-Regular.ttf", (255, 0, 0), 45, 600, 890, counter, 60)
        return self.surface
